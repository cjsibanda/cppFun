...
namespace sibanda {
 ...
 {
  m_timedEnd = std::chrono::steady_clock::now();
 }

 //record an event
 void TimeTask::addTask(const char* msg)
 {
    auto ms = std::chrono::duration_cast<std::chrono::nanoseconds>(m_timeEnd - m_timedStart);
    if (m_cnt < MAX_EVENTS)
    {
