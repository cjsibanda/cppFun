...
namespace mandoza {
    void Kunye::cpyName(const char* src) {
        ...
        m_holderName[i] = char(0); // null-terminate
    }
    ...
    //Valid Kunye: 5-digit number, non-negative balance, non-empty name
    ...
    //Transfer all the funds: right -> this
    ...
    //Transfer all the funds: this -> right
    Kunye& Kunye::operator>>(Kunye& right) {
        if (this != &right && *this && right) {
