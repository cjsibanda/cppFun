...
// To check if header guards exist and follow convention.
...
int cout = 0; // --> this won't compile if headers don't follow convention
...
int main(int argc, char** argv)
{
	...
	// The data that will be used by the template classes
	long   iData[]{    16,    12,     9,      3,    21,    12, 23 };
	double dData[]{ 1.222, 5.777, 1.223, 51.333, 8.888, 1.221 };
	sibanda::Pair pData[]{
		sibanda::Pair("static memory", "memory allocated during the compilation time"),
		sibanda::Pair("programmer", "a person who writes code"),
		sibanda::Pair("dynamic memory", "memory allocated during the runtime"),
		sibanda::Pair("client of a class", "a piece of code that uses the class"),
		sibanda::Pair("dynamic memory", "memory allocated with 'new' operator"),
		sibanda::Pair("user", "a person who runs and interracts with the program"),
		sibanda::Pair("array", "a contiguous block of memory storing multiple objects"),
		sibanda::Pair("client of a function", "a piece of code that uses the function"),
		sibanda::Pair("user", "Hello Mr. Sibanda!"),
	};

	{
		std::cout << ">>>  T1: Testing the Collection of Integers\n";
		std::cout << "==========:==========:==========:==========:==========\n";
		sibanda::Collection<long, 20> colIntegers;
...

	{
		...
		std::cout << "==========:==========:==========:==========:==========\n";
		sibanda::Collection<long, 100>* setIntegers = new sibanda::Set<long>();
...

	{
		...
		std::cout << "==========:==========:==========:==========:==========\n";
		sibanda::Collection<double, 30> colFloats;
...

	{
		...
		std::cout << "==========:==========:==========:==========:==========\n";
		sibanda::Collection<double, 100>* setFloats = new sibanda::Set<double>;
...

	{
		...
		std::cout << "==========:==========:==========:==========:==========\n";
		sibanda::Collection<sibanda::Pair, 50> colDictionary;
...

	{
		...
		std::cout << "==========:==========:==========:==========:==========\n";
		sibanda::Collection<sibanda::Pair, 100>* setDictionary = new sibanda::Set<sibanda::Pair>();
