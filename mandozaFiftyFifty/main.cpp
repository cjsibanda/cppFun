...
// Only for testing/eductional purposes

#include <iostream>
#include "Kunye.h"
using namespace std;
using namespace mandoza;
void displayAccounts(const Kunye* acc, int num,int startFrom = 0);
void displayAccount(const Kunye& acc);
void displayRawAccountNumber(const Kunye* acc, int num);
void displayRawBalances(const Kunye* acc, int num);
int countNewAccounts(const Kunye* acc, int num);
int countBadAccounts(const Kunye* acc, int num);
void prnIntials(const Kunye& A);
void toUpper(char& ch);
void capitalize(Kunye* Arr, int size);
void capitalize(Kunye& A);
void prnIntials(const Kunye& A);
void toUpper(char& ch);
void capitalize(Kunye* Arr, int size);
void capitalize(Kunye& A);
...
int main() {
   double value;
   Kunye mixed[MixedSize] = {
      {"David Bhekham", 11111, 111.11},
      {"Lebron Jim", 22222, 222.22},
      {"Peter Steph", 333, 333.333},   
      {"Sasha Feinburg", 44444, 4444.44},
      {"Kingsley Tshabalala", 55555, 555.55},
      {"Peter Ndhlovu", 66666, -66.66},  // 
      {"Lindsey Vought",77777, 777.77},
      {"Hebert Nyoni XholisaNgulube Jr.", 80000, 888.00},
      {"North South" },
      {"John Hadebe"},
...
void displayAccounts(const Kunye* acc, int num, int startFrom ) {
   cout << "+---+-----+--------------------------------+-------+--------------+" << endl;
